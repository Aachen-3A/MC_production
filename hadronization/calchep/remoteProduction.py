...
uberftp grid-ftp.physik.rwth-aachen.de "cd /pnfs/physik.rwth-aachen.de/cms/store/user/$user/MC/%s; get $file"
...
def handle_LHEs(options, args):
    ...
    for file in lhe_file_list:
        path = "srm://grid-srm.physik.rwth-aachen.de:8443/pnfs/physik.rwth-aachen.de/cms/store/user/%s/MC/%s/"%(options.user, options.Output)
        cmd1 = "srmcp"
...
def main():
    ...
    lhe_file_list = get_lhe_list(options, args)
    ...
    task=cesubmit.Task('LHE_hadronization_%s'%options.Output,scramArch=scram_arch, cmsswVersion=cmssw_version)
